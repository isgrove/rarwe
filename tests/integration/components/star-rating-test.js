...
module('Integration | Component | star-rating', function (hooks) {
  ...
  test('Renders the full and empty stars correctly', async function (assert) {
    ...
    assert
      ...
      .exists(
        { count: 2 },
        'The right amount of full stars is rendered after chaning'
      );
    assert
      .dom('[data-test-rr="empty-star"]')
      .exists(
        { count: 3 },
        'The right amount of empty stars is rendered after chaning'
      );
  });

  test('Calls onUpdate with the correct values', async function (assert) {
    this.set('rating', 2);
